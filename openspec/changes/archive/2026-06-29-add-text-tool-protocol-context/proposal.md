...
- `text` protocol では provider request payload から native `tools` を外す。agent execution context の active tools は残す。
...
- system prompt に strict text tool protocol の出力規約を追加する。`auto` では text を fallback format として説明し、native Tool Use を使えるモデルに text call を優先させない。
...
- 推奨実装順の 2 番目。`add-strict-text-tool-call-extraction` に依存し、`expose-tool-call-protocol-setting` と `add-auto-tool-protocol-fallback` の前提になる。
